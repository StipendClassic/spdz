...
*Setup instructions for a gitian build of SPDZ using a Debian VM or physical system.*
...
Clone the git repositories for SPDZ and gitian and then checkout the SPDZ version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/SPDZ-crypto/SPDZ.git
cd SPDZ
...
Follow the instructions in [doc/release-process.md](release-process.md) in the SPDZ repository
...
To build SPDZ (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the SPDZ repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/SPDZ/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/SPDZ-crypto/SPDZ
...
For example:
```bash
URL=https://github.com/crowning-/SPDZ.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/SPDZ-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/SPDZ-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/SPDZ-build.assert
```
...
[SPDZ/gitian.sigs](https://github.com/SPDZ-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
