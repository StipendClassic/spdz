...
// Copyright (c) 2015-2018 The SPDZ developers
...
int64_t GetBlockValue(int nHeight)
{
	...
	nSubsidy = 3 * COIN;
    } else if (nHeight < 86400 && nHeight > 0) { //POW ends at Block 500
        nSubsidy = 25 * COIN;
        ...
        nSubsidy = 4 * COIN; //Halves until max
    }
    return nSubsidy;
}
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    int64_t mNodeCoins = nMasternodeCount * 7000 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSPDZStake)
{
    int64_t ret = 0;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSPDZSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSPDZ money supply in the block index
    if (!UpdateZSPDZSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zSPDZ supply for block=%s height=%d", __func__,
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSPDZ stake block is less than allowed start height", __func__);

    if (CZSPDZStake* zSPDZ = dynamic_cast<CZSPDZStake*>(stake)) {
        CBlockIndex* pindexFrom = zSPDZ->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSPDZ stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSPDZ stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSPDZ->GetValue()));
        if (nChecksum200 != zSPDZ->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSPDZ->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSPDZ() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSPDZ fails context checks", __func__));
