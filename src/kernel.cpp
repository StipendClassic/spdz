...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSPDZStake(spend));
    } else {
        ...
        CSPDZStake* SPDZInput = new CSPDZStake();
        SPDZInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(SPDZInput);
    }
